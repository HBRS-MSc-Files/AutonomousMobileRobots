import unittest
import numpy as np
from KalmanFilter import KalmanFilter  # Assuming your class is in State.py

class TestState(unittest.TestCase):
    def test_default_initialization(self):
        """Test default initialization with zero mean and identity covariance."""
        state = KalmanFilter()
    ...
    def test_set_and_get_state_estimate(self):
        """Test setting and getting the mean."""
        state = KalmanFilter()
        new_mean = [1, 2, 3]
    ...
    def test_set_and_get_error_covariance(self):
        """Test setting and getting the covariance."""
        state = KalmanFilter()
    ...
    def test_state_estimate_update(self):
        """Test mean update with identity motion model."""
        state = KalmanFilter()
        A = np.eye(3)  
        B = np.eye(3)  
        u = np.array([1, 0, 0]) 
    ...
    def test_error_covariance_after_state_estimate_update(self):
        """Test covariance update with small process noise."""
        state = KalmanFilter()
        A = np.eye(3)  
        B = np.eye(3) 
        u = np.array([0, 0, 0])  
    ...
    def test_no_movement(self):
        """Test that no movement or noise results in no change."""
        state = KalmanFilter()
        A = np.eye(3)  
        B = np.eye(3)  
        u = np.array([0, 0, 0])  
    ...
    def test_basic_update(self):
        """Test state update with simple measurements."""
        state = KalmanFilter()
        H = np.eye(3)  
        R = np.eye(3) * 0.1  
        z = np.array([1, 1, 0])  
    ...
    def test_error_covariance_after_measurement_update(self):
        """Test that covariance decreases after update."""
        state = KalmanFilter()
        H = np.eye(3) 
        R = np.eye(3) * 0.1 
        z = np.array([1, 1, 0]) 
        ...
        self.assertTrue(np.all(updated_covariance < initial_covariance))
